...
const defaultSettings = {
  ...
  temperature: clamp(
    toFloat(injectedDefaults.temperature, FALLBACK_SETTINGS.temperature),
    0,
    1
  ),
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadTemplates() {
    ...
    Object.keys(savedTemplates).forEach((key) => {
      const label = seen.has(key)
        ? `${key} (замена)`
        : `${key} (пользовательский)`;
  ...
  function applySettingsToForm(settings) {
    ...
    if (customModelDiv) {
      customModelDiv.style.display =
        settings.model === "custom" ? "block" : "none";
    }
  }
  ...
  async function syncSettingsWithServer(settings) {
    try {
      ...
      if (data.status === "success") {
        ...
        throw new Error(
          data.message || "Не удалось синхронизировать настройки"
        );
      }
    } catch (error) {
      logMessage(
        `Предупреждение: не удалось синхронизировать с сервером (${error.message})`,
        "warning"
      );
    }
  }
...
async function fetchServerSettings() {
  try {
    ...
    logMessage(
      `Не удалось загрузить настройки с сервера: ${error.message}`,
      "warning"
    );
  }
  return null;
...
function normalizeSettings(settings = {}) {
  return {
    ...
    temperature: clamp(
      toFloat(settings.temperature, defaultSettings.temperature),
      0,
      1
    ),
  };
...
function persistLocalSettings(settings) {
  try {
    ...
    logMessage(
      `Не удалось сохранить настройки в браузере: ${error.message}`,
      "warning"
    );
  }
}
...
function readLocalSettings() {
  try {
    ...
    logMessage(
      `Не удалось прочитать настройки из браузера: ${error.message}`,
      "warning"
    );
    return null;
  }
...
function writeUserTemplates(data) {
  try {
    ...
    logMessage(
      `Не удалось сохранить шаблон в браузере: ${error.message}`,
      "warning"
    );
  }
}
...
function getCurrentSettings() {
  const local = readLocalSettings();
  const source =
    cachedSettings || (local ? normalizeSettings(local) : defaultSettings);
